...
public class ASBoss extends AnimatedSprite {
	private List<ASBossClaw> bossClaws;
	private int healthAtStart;
	private List<BossShield> bossShield;
	...
	public List<ASBossClaw> getBossClaws(){
		return bossClaws;
	}
	
	public List<BossShield> getBossShield(){
		return bossShield;
	}
	...
	public static List<ASBoss> generateBoss(int amount, int health, double posX, double posY){
		List<ASBoss> boss = new ArrayList<ASBoss>();
	...
	public static void regenerateBoss(List<ASBoss> boss, int amount, int health, double posX, double posY) {
	...
	//Behavior
	public void setBossBehavior(List<AnimatedSprite> smallAsteroidList) {
		setTime(getTime() + 1);
		if(bossClaws.size() == 0) {
			bossShield.clear();
		}
	...
	public void updateAndRender(double time, GraphicsContext gc, List<ASEnemySpaceship> enemies, List<AnimatedSprite> smallAsteroidList) {
