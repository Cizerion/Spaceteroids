...
public class ASSpaceship extends AnimatedSprite {
	...
	public void setDamagedShipFrames() {
		...
		for(int i = 0; i < imageArray.length; i++) {
			if((i % 2) == 0) {
				...
				imageArray[i] = new Spaceship("images/ship/damaged.png", 64, 29, true, true, getPositionX(), getPositionY(), getVelocityX(), getVelocityY());
			}
		}
	...
	public void setImmunity(boolean imnty) {
		immunity = imnty;
		if(immunity) {
			setDamagedShipFrames();
		} else {
			setNotDamagedShipFrames();
		}
	}
	...
	public void reloading(List<Sprite> laserTempList) {
	...
	public void addLaser(List<Sprite> laserTempList) {
	...
	public void updateAndRender(double time, GraphicsContext gc, List<Sprite> laserTempList) {
		firerate++;
		for(Sprite tempLaser: laserList) {
		...
		if(getHealth() >= 0 && getHealth() <= 100) {
			gc.drawImage(healthBar[(getHealth()/25)].getImage(), 200, 20);
		} else {
			gc.drawImage(steelHealthBar.getImage(), 200, 20);
		}
